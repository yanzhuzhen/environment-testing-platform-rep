          ...
          <el-tabs style="height: 633px;"  type="card">
            ...
            <el-tab-pane label="我的文章">
              <el-table :height="tableHeight" :data="articleList" border fit highlight-current-row style="width: 100%">
                <el-table-column width="180px" align="center" label="文章编号" >
                  <template v-slot="scope">
                ...
                <el-table-column min-width="100px" label="文章标题" >
                  <template v-slot="scope">
                ...
                <el-table-column label="文章状态" >
                  <template v-slot="scope">
                    <el-button type="primary" v-if="scope.row.status === 'draft'" round size="small">草稿</el-button>
                    <el-button type="warning" v-if="scope.row.status === 'published'" round size="small">已发布</el-button>
                    <el-button type="info" v-if="scope.row.status === 'enable'" round size="small">已审核</el-button>
                  </template>
                </el-table-column>
                <el-table-column label="是否开启评论" >
                  <template v-slot="scope">
                    <el-button type="success" v-if="scope.row.comment_disable === 0" round size="small">已打开</el-button>
                    <el-button type="danger" v-if="scope.row.comment_disable === 1" round size="small">未打开</el-button>
                  </template>
                </el-table-column>
                <el-table-column min-width="100px" label="查看文章" >
                  <template v-slot="scope">
                    <span @click="upto(scope.row.id)">查看文章</span>
                  </template>
                </el-table-column>
...
export  default {
  ...
  methods:{
    ...
    async search(pageNow = 1, pageSize = 10) {
      ...
      //发送查询请求
      let res1 = await alg.getAlgList(this.searchModel);
      if (res1.success) {
