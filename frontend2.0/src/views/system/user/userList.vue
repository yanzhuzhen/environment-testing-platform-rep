<template>
  <el-main>
    <el-form :model="searchModel" ref="searchForm" label-width="80px" :inline="true" size="small">
      <el-form-item>
        <el-input v-model="searchModel.username" placeholder="请输入用户名查询"/>
        <el-input v-model="searchModel.realname" placeholder="请输入真实姓名查询"/>
        <el-input v-model="searchModel.phone" placeholder="请输入联系电话查询"/>
      </el-form-item>
      <el-form-item>
    ...
    <system-dialog :title="userDialog.title" :visible="userDialog.visible" :width="userDialog.width" :height="userDialog.height"
      ...
      <div slot="content">
        <el-form ref="userForm" :model="user" :rules="rules" label-width="80px" :inline="true" size="small">
          ...
          <el-form-item label="密码" size="small" prop="password" v-if="user.id === ''">
...
<!--分页工具栏-->
      <el-pagination
        ...
        :currentypage.sync="roleVo.pageNo"
...
export default {
    ...
    methods: {
      ...
      async handleDelete(row) {
        ...
        if (confirm) {
          let res = user.deleteUser({id: row.uno});
          if (res.success) {
            this.$message.success(res.message);
            await this.search(this.pageNow, this.pageSize);
          } else {
      ...
      async assignRole(row) {
        ...
        //发送根据用户ID查询角色列表的请求
        let res = await user.getRnoByUno(params);
        //如果存在数据
